...
@Controller
@RequestMapping("HisStatisticalInfoController")
public class HisStatisticalInfoController {

	@Autowired
	HisStatisticalInfoService hisStatisticalInfoService;
	
	@ResponseBody
	@RequestMapping(value = "getHisSta", method = RequestMethod.POST)
	public List<HisImportantData> getHisSta(@RequestParam("startTime") String startTimeStr, @RequestParam("endTime") String endTimeStr, @RequestParam("selectValue") String[] selectValue) throws ParseException{
...
//		List<HisImportantData>[] data = new List<HisImportantData>[selectValue.length];
		...
		for (int i = 0; i < selectValue.length; i++){
			...
			data[i] = hisStatisticalInfoService.getHisSta(queryPeriod);
