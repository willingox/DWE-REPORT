...
@Controller
@RequestMapping("ReportYCImportantMeasureController")
public class ReportYCImportantMeasureController {

	@Autowired
	ReportYCImportantMeasureService reportYCImportantMeasureService;
	
	@ResponseBody
	@RequestMapping(value = "getYCImport", method = RequestMethod.POST)
	public List<YCReportData> getHisImportant(@RequestParam("startTime") String startTimeStr, @RequestParam("endTime") String endTimeStr, @RequestParam("selectValue") String[] selectValue, @RequestParam("ids") int[] ids) throws ParseException{
...
//		List<YCReportData> data[] = new ArrayList[selectValue.length];
...
//			data[i] = reportYCImportantMeasureService.getYCImport(queryPeriod);
			
		}
		queryPeriod.setSql(sql);
		yCReportData = reportYCImportantMeasureService.getYCImport(queryPeriod);
