...
@Controller
@RequestMapping("HisMeasureInfoController")
public class HisMeasureInfoController {

	@Autowired
	HisMeasureInfoService hisMeasureInfoService;
	
	@ResponseBody
	@RequestMapping(value = "getHisMea", method = RequestMethod.POST)
	public List<HisImportantData> getHisMea(@RequestParam("startTime") String startTimeStr, @RequestParam("endTime") String endTimeStr, @RequestParam("selectValue") String[] selectValue) throws ParseException{
...
//		List<HisImportantData>[] data = new List<HisImportantData>[selectValue.length];
		...
		for (int i = 0; i < selectValue.length; i++){
			...
			data[i] = hisMeasureInfoService.getHisMea(queryPeriod);
